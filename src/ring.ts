import type { AppState, DomRefs } from "./types.js";
...
export interface RingApi {
  setupRing: () => void;
  ringFrac: () => number;
  setRingImmediate: (frac: number) => void;
  startRingEpoch: (fromFrac: number, toFrac: number) => void;
}

export function createRing(state: AppState, dom: DomRefs): RingApi {
  ...
  function drawTicks() {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const major = i % 5 === 0;
      h += `<line x1="${CX + oR * Math.cos(a)}" y1="${CY + oR * Math.sin(a)}" x2="${
        CX + iR * Math.cos(a)
      }" y2="${CY + iR * Math.sin(a)}" stroke="rgba(255,255,255,${major ? 0.22 : 0.09})" stroke-width="${
        major ? 2 : 1
  ...
  function setRingImmediate(frac: number) {
    if (ring.rafId != null) cancelAnimationFrame(ring.rafId);
    ring.rafId = null;
    ring.startFrac = frac;
    ring.endFrac = frac;
    dom.ringProg.setAttribute("stroke-dashoffset", String(CIRC * (1 - frac)));
  ...
  function startRingEpoch(fromFrac: number, toFrac: number) {
    ring.startFrac = fromFrac;
    ring.endFrac = toFrac;
    ring.epochStart = performance.now();
    if (!ring.rafId) rafLoop();
  }
