import type { HistoryEntry, HistoryApi } from "./types.js";
...
interface HistoryItemWithMeta extends HistoryEntry {
  bucket: Period["key"];
  hourLabel: string;
}
...
const EARLY_SLOT: Period["key"] = "morning";

let selectedDay: string | null = null;
const openPeriodsByDay = new Map<string, Set<Period["key"]>>();

function fmt(secs: number): string {
  return `${String(Math.floor(secs / 60)).padStart(2, "0")}:${String(secs % 60).padStart(2, "0")}`;
}

function parseHist(): HistoryEntry[] {
  try {
    return JSON.parse(localStorage.getItem("ff_hist") || "[]") as HistoryEntry[];
  } catch {
    return [];
  }
}

function saveHist(next: HistoryEntry[]): void {
...
function groupedHistory(): DayGroup[] {
  ...
  all.forEach((item) => {
    ...
    if (item.phase === "focus") group.focusTotal += Number(item.secs) || 0;
  });

  return Array.from(groups.values()).sort((a, b) => b.day.localeCompare(a.day));
}

function ensureOpenPeriods(day: string): Set<Period["key"]> {
...
export function initHistoryMenu(options: HistoryMenuOptions): HistoryApi {
  const { daysEl, detailEl, clearEl } = options;

  function renderDays(groups: DayGroup[]): void {
    daysEl.innerHTML = "";
    groups.forEach((group, index) => {
      const active = selectedDay ? selectedDay === group.day : index === 0;
      if (active) selectedDay = group.day;
  ...
  function renderDetail(groups: DayGroup[]): void {
  ...
  function removeHistoryItem(ts: number): void {
  ...
  daysEl.addEventListener("click", (event) => {
    const btn = (event.target as Element | null)?.closest(".hist-tab") as HTMLButtonElement | null;
    if (!btn) return;
  ...
  detailEl.addEventListener("click", (event) => {
    const del = (event.target as Element | null)?.closest(".hist-del") as HTMLButtonElement | null;
    if (del) {
      event.stopPropagation();
      const ts = Number(del.dataset.ts || 0);
      if (ts) removeHistoryItem(ts);
      return;
    }

    const periodBtn = (event.target as Element | null)?.closest(".hist-period-head") as HTMLButtonElement | null;
    if (periodBtn) {
      if (!selectedDay) return;
      const set = ensureOpenPeriods(selectedDay);
      const period = periodBtn.dataset.period as Period["key"] | undefined;
      if (!period) return;
    ...
    const item = (event.target as Element | null)?.closest('.hist-item[data-kind="focus-item"]') as HTMLElement | null;
    if (item) {
      const secs = Number(item.dataset.secs || 0);
      if (secs > 0) options.onApplyDuration(secs);
    }
  });

  detailEl.addEventListener("keydown", (event) => {
    const item = (event.target as Element | null)?.closest('[data-menu-item="true"]') as HTMLElement | null;
    if (!item || event.key !== "Enter") return;
    event.preventDefault();
    item.click();
  });
  ...
  function focusItems(): HTMLElement[] {
    ...
    return roots.flatMap((root) => Array.from(root.querySelectorAll<HTMLElement>('[data-menu-item="true"]')));
  }

  function selectPrevDay(): void {
    const tabs = Array.from(daysEl.querySelectorAll<HTMLButtonElement>(".hist-tab"));
  ...
  function toggleFocusedPeriod(direction: -1 | 1): void {
    const active = document.activeElement;
    if (!(active instanceof HTMLElement)) return;
    const periodBtn = active.closest(".hist-period-head") as HTMLButtonElement | null;
    if (!periodBtn || !selectedDay) return;
    const period = periodBtn.dataset.period as Period["key"] | undefined;
    if (!period) return;
    const set = ensureOpenPeriods(selectedDay);
    if (direction > 0) set.add(period);
